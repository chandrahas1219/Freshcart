"""Admin portal: gated registration, login, profile, customer/order
...
from excel_helpers import (
    ADMINS_FILE, CUSTOMERS_FILE, GROCERIES_FILE,
    get_all_rows, get_row_by_id, get_row_by_email,
    create_record, update_row, parse_transaction_history,
)
...
# ---------------------------------------------------------------------------
# Registration gate + registration + login
# ---------------------------------------------------------------------------

@admin_bp.route("/verify-key", methods=["GET", "POST"])
def verify_key():
    """The secret-key gate that must be passed before the registration
    form is shown, per the system's admin-onboarding requirement."""
...
def profile():
    ...
    if request.method == "POST":
        ...
        if not name or not email:
            ...
            flash("That email is already in use by another admin.", "error")
        else:
            update_row(ADMINS_FILE, "AdminID", session["admin_id"], {
...
def _parse_price_and_quantity(form):
    """Returns (price, quantity, error_message). error_message is None
...
def inventory_add():
    ...
    if not name or not unit:
        ...
        create_record(GROCERIES_FILE, "ItemID", {
            "Name": name, "Category": category, "Unit": unit,
            "PricePerUnit": price, "QuantityInStock": quantity,
            "ImageURL": image_url,
        })
...
def inventory_edit(item_id):
    ...
    if request.method == "POST":
        ...
        if not name or not unit:
            ...
            update_row(GROCERIES_FILE, "ItemID", item_id, {
                "Name": name, "Category": category, "Unit": unit,
                "PricePerUnit": price, "QuantityInStock": quantity,
                "ImageURL": image_url,
            })
        ...
        # fall through and re-show the form with the attempted values

...
def chat_message():
    ...
    try:
        ...
        # Step 4: Remember this turn for next time (short reply only, not
        # the full itemized message, to keep future context compact)
        append_chat_message(customer_id, "user", user_prompt)
        assistant_text = parsed.get("reply") or response.get("message", "")
        if assistant_text:
